...
export default function App() {
  ...
  // Collapsed state for live packlists
  const [floatingPacklistCollapsed, setFloatingPacklistCollapsed] = useState(false);
  const [compactPacklistCollapsed, setCompactPacklistCollapsed] = useState(false);
  ...
  return (
    <div data-unused={__unused_refs.length} style={{ width: '100vw', height: '100vh', background: '#f0f0f0', position: 'relative' }}>
      {/* Floating live packlista (always visible) */}
      <div id="packlista-floating" style={{ position: 'fixed', left: 320, top: 12, width: 200, padding: 8, background: '#fff', border: '1px solid #ddd', borderRadius: 6, boxShadow: '0 6px 20px rgba(0,0,0,0.12)', zIndex: 1201 }}>
  ...
  {/* (Debug-info removed) */}
        ...
        {floorIndex !== null && FLOOR_SIZES[floorIndex]?.custom && (
          ...
          {/* Compact live packlista (provisorisk) */}
          <div style={{ marginTop: 12, padding: 8, background: '#fff', border: '1px solid #ddd', borderRadius: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              <div style={{ fontWeight: 700 }}>Packlista (live)</div>
              <button
                aria-label={compactPacklistCollapsed ? 'Visa packlista (kompakt)' : 'Minimera packlista (kompakt)'}
                title={compactPacklistCollapsed ? 'Visa' : 'Minimera'}
                onClick={() => setCompactPacklistCollapsed(!compactPacklistCollapsed)}
                style={{ width: 22, height: 22, padding: 0, borderRadius: '50%', border: 'none', background: '#007acc', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, lineHeight: '16px', cursor: 'pointer', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', zIndex: 1302 }}
              >{compactPacklistCollapsed ? '+' : '−'}</button>
            </div>
            {!compactPacklistCollapsed && (() => {
              const floorConfig = floorIndex !== null ? FLOOR_SIZES[floorIndex] : null;
              const floorW = floorConfig?.custom ? customFloorWidth : (floorConfig ? floorConfig.width : 0);
              const floorD = floorConfig?.custom ? customFloorDepth : (floorConfig ? floorConfig.depth : 0);
              const pack = computePacklista(wallShape, floorW, floorD, wallHeight, storages);
              const totals = Object.assign({}, pack.totals || {});
              // add TV counts to live packlista
              if ((tvs || []).length > 0) {
                const tvCounts: Record<string, number> = {};
                (tvs || []).forEach(tv => {
                  const label = TV_SIZES[tv.size]?.label || 'Okänd';
              ...
              // add plant counts to compact live packlista
              if ((plants || []).length > 0) {
                const plantCounts: Record<string, number> = {};
                (plants || []).forEach(p => {
                  const label = PLANT_TYPES[p.type]?.label || 'Okänd';
              ...
              // Ensure disk innehylla from placed counters is shown in compact live totals
              try {
                const diskCount = (counters || []).length * 2;
                if (diskCount > 0) (totals as any)['disk innehylla'] = ((totals as any)['disk innehylla'] || 0) + diskCount;
              } catch (e) {
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  {miscKeys.map(k => {
                    ...
                    const display = (typeof v === 'number') ? `${v}st` : `${v}`;
                    return (
                      <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                        <div style={{ color: '#444' }}>{k.replace('_', ' ')}</div>
                        <div style={{ color: '#007acc', fontWeight: 700 }}>{display}</div>
                      </div>
                    );
                  })}
                  {([].concat(...Object.keys(pack.perWall || {}).map(k => (pack.perWall[k].storages || []))) || []).map((s: any) => (
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        // Compute packlista and render hidden DOM block to canvas
                        try {
                          const floorConfig = floorIndex !== null ? FLOOR_SIZES[floorIndex] : null;
                          const floorW = floorConfig?.custom ? customFloorWidth : (floorConfig ? floorConfig.width : 0);
                          const floorD = floorConfig?.custom ? customFloorDepth : (floorConfig ? floorConfig.depth : 0);
  ...
  {/* 3D-scen */}
  <div style={{ marginLeft: 300, width: 'calc(100vw - 300px)', height: '100vh', position: 'relative' }}>
        {floorIndex !== null && (() => {
